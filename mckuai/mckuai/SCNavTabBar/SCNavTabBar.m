...
//    [_items[0] setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
...
- (void)itemPressedSwift:(NSInteger)currentIndex
{
    [_delegate itemDidSelectedWithIndex:currentIndex];
}
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    for (NSString *title in titles)
    {
        ...
        NSNumber *width = [NSNumber numberWithFloat:75.0f];
...
{
    NSLog(@"%ld",(long)currentItemIndex);
    
    for (NSInteger index = 0; index < [_itemsWidth count]; index++) {
        if (index != currentItemIndex) {
//            [_items[index] setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            UIButton *otherButton = _items[index];
